...
def excel_to_csv():
    ...
    if len(column_name_list) != len(df.columns):
        print(f"カラム名の数が一致しません: {len(column_name_list)} vs {len(df.columns)}")
        return
    # カラム名を設定
    ...
    print(f"{excel_path} を {save_path} に保存しました。")
