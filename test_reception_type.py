...
from src.checklist.automation.check_functions import check_reception_type

def test_check_reception_type():
    print("=== 受付種別チェック関数のテスト ===")
    ...
    print(f"データ形状: {integrated_club_data.shape}")
    print(f"クラブ名の一例: {integrated_club_data['クラブ名'].head()}")
    
    # テスト用のクラブ名
    test_club_name = "日本橋クラブ"
    test_date = "2025-06-05 17:29:33.531"
    
    print(f"\n2. テスト対象: {test_club_name}, {test_date}")
    ...
    # check_reception_type関数をテスト
    print(f"\n3. check_reception_type関数の実行...")
    result = check_reception_type(target_row, integrated_club_data, test_club_name)
    
    print(f"結果: {result}")

if __name__ == "__main__":
    test_check_reception_type()
