...
from src.checklist.automation.check_functions import check_number_of_members

def test_check_number_of_members():
    print("=== 会員数チェック関数のテスト ===")
    ...
    # check_number_of_members関数をテスト
    print(f"\n3. check_number_of_members関数の実行...")
    result = check_number_of_members(target_row)
    
    print(f"結果: {result}")
    
    if not result:
        print("✅ 会員数チェックが正常に通りました（エラーなし）")
    else:
        print("❌ 会員数チェックでエラーが発生しました")

if __name__ == "__main__":
    test_check_number_of_members()
