...
def check_application_type(application_row, application_data_df, club_name):
    ...
    if not application_row.empty:
        # まずクラブ名を正規化
        application_data_df['クラブ名'] = application_data_df['クラブ名'].astype(str).str.strip()
        club_name = str(club_name).strip()
        # ここで抽出
        application_data_match_row = application_data_df[application_data_df['クラブ名'] == club_name]
        print("application_data_match_row columns:", application_data_match_row.columns.tolist())
        print("application_data_match_row head:", application_data_match_row.head())
        # カラム名のバリエーションに対応
        r7_col = None
        for col in application_data_match_row.columns:
            if col.startswith('R7年度登録クラブ'):
                r7_col = col
                break
        if r7_col is None:
            print("R7年度登録クラブカラムが見つかりません:", application_data_match_row.columns.tolist())
            error_dict['e_a_005-d'] = 'R7年度登録クラブカラムが見つかりません'
            return error_dict
        # 以降は r7_col を使って参照
        if application_row['申請_申請種別'].iloc[0] == '新規（R7年度には登録していない）' and application_data_match_row[r7_col].iloc[0] == 1:
            error_dict['e_a_005-a'] = 'R7年度の登録クラブだが、新規登録として申請されている'
        elif application_row['申請_申請種別'].iloc[0] == '更新（R7年度に登録済み）' and application_data_match_row[r7_col].iloc[0] == 0:
