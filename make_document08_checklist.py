def make_document08_checklist(latest_reception_data_date):
    ...
    import pandas as pd
    import logging
    from setting_paths import content_check_folder_path, document08_checklist_folder_path, clubs_reception_data_path
    ...
    document08_checklist_columns_file_name = 'document08_checklist_columns.json'
    document08_checklist_columns_file_path = os.path.join(content_check_folder_path, document08_checklist_columns_file_name)
    ...
    logging.info(f"書類08のチェックリストのカラム名を読み込みました: {document08_checklist_columns_file_name}")
    ...
    # 5. 書類08のチェックリストのデータフレームを保存(ファイル名は「書類08チェックリスト_受付{latest_reception_data_date}_作成{YYYYMMDDHHMMSS}.xlsx」)
    logging.info("書類08のチェックリストのデータフレームを保存します")
    now_jst = get_jst_now()
    document08_checklist_file_name = f'書類08チェックリスト_受付{latest_reception_data_date}_作成{now_jst.strftime("%Y%m%d%H%M%S")}.xlsx'
    document08_checklist_file_path = os.path.join(document08_checklist_df, document08_checklist_file_name)
