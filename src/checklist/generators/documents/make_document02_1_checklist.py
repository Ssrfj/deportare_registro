def make_document02_1_checklist(latest_reception_data_date):
    ...
    # 5. 書類02_1のチェックリストのデータフレームを保存(ファイル名は「書類02_1チェックリスト_受付{latest_reception_data_date_str}_作成{YYYYMMDDHHMMSS}.xlsx」)
    logging.info("書類02_1のチェックリストのデータフレームを保存します")
    now_jst = get_jst_now()
    document02_1_checklist_file_name = f'書類02_1チェックリスト_受付{latest_reception_data_date_str}_作成{now_jst.strftime("%Y%m%d%H%M%S")}.xlsx'
