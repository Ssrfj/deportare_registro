def make_document05_plan_checklist(latest_reception_data_date):
    ...
    document05_plan_checklist_columns_file_path = get_config_file_path('config/checklist_columns/document05_plan_checklist_columns.json')
    ...
    # 5. 書類05_計画のチェックリストのデータフレームを保存(ファイル名は「書類05_計画チェックリスト_受付{latest_reception_data_date_str}_作成{YYYYMMDDHHMMSS}.xlsx」)
    logging.info("書類05_計画のチェックリストのデータフレームを保存します")
    now_jst = get_jst_now()
    document05_plan_checklist_file_name = f'書類05_計画チェックリスト_受付{latest_reception_data_date_str}_作成{now_jst.strftime("%Y%m%d%H%M%S")}.xlsx'
