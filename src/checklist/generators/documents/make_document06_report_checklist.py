def make_document06_report_checklist(latest_reception_data_date):
    ...
    import pandas as pd
    import logging
    from src.core.setting_paths import content_check_folder_path, document06_report_checklist_folder_path, clubs_reception_data_path
    ...
    document06_report_checklist_columns_file_name = get_config_file_path('config/checklist_columns/document06_report_checklist_columns.json')
    ...
    logging.info(f"チェックリストのカラム名を読み込みました: {document06_report_checklist_columns_file_path}")
    ...
    # 5. 書類06_報告のチェックリストのデータフレームを保存(ファイル名は「書類06_報告チェックリスト_受付{latest_reception_data_date_str}_作成{YYYYMMDDHHMMSS}.xlsx」)
    logging.info("書類06_報告のチェックリストのデータフレームを保存します")
    now_jst = get_jst_now()
    document06_report_checklist_file_name = f'書類06_報告チェックリスト_受付{latest_reception_data_date_str}_作成{now_jst.strftime("%Y%m%d%H%M%S")}.xlsx'
    document06_report_checklist_file_path = os.path.join(document06_report_checklist_folder_path, document06_report_checklist_file_name)
