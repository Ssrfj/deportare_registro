def make_document04_checklist(latest_reception_data_date):
    ...
    import pandas as pd
    import logging
    from setting_paths import content_check_folder_path, document04_checklist_folder_path, clubs_reception_data_path
    ...
    document04_checklist_columns_file_name = 'document04_checklist_columns.json'
    ...
    logging.info(f"書類04のチェックリストのカラム名を読み込みました: {document04_checklist_columns_file_name}")
    ...
    # 5. 書類04のチェックリストのデータフレームを保存(ファイル名は「書類04チェックリスト_受付{latest_reception_data_date}_作成{YYYYMMDDHHMMSS}.xlsx」)
    logging.info("書類04のチェックリストのデータフレームを保存します")
    now_jst = get_jst_now()
    document04_checklist_file_name = f'書類04チェックリスト_受付{latest_reception_data_date}_作成{now_jst.strftime("%Y%m%d%H%M%S")}.xlsx'
    document04_checklist_file_path = os.path.join(document04_checklist_df, document04_checklist_file_name)
