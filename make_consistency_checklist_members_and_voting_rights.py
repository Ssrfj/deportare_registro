def make_consistency_checklist_members_and_voting_rights(latest_reception_data_date):
    ...
    import pandas as pd
    import logging
    from setting_paths import content_check_folder_path, consistency_checklist_members_and_voting_rights_folder_path
    ...
    consistency_checklist_members_and_voting_rights_columns_file_path = os.path.join(content_check_folder_path, consistency_checklist_members_and_voting_rights_columns_file_name)
    ...
    for index, row in club_reception_df.iterrows():
        ...
        consistency_checklist_members_and_voting_rights_df.loc[index, 'チェック者名_会員と議決権保有者'] = 'チェックが完了していません'
    logging.info("会員と議決権保有者の一貫性のチェックリストのデータフレームを作成しました")
